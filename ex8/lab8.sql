...
declare 
e_pid products.pid%type;
rid receipts.rno%type;

begin
	rid := &receipt_no;
	begin
            select item into e_pid from item_list where rno = rid;
	EXCEPTION 
	   WHEN NO_DATA_FOUND then
	       dbms_output.put_line('NO SUCH DATA');
	   RETURN;
	   WHEN TOO_MANY_ROWS THEN
	        DBMS_OUTPUT.PUT_LINE('More than one receipts were found for '||rid);
	   RETURN;
        end;
if SQL%ROWCOUNT=1 then
	DBMS_OUTPUT.PUT_LINE('One receipt was found for '||rid);
end if;
end;
/
